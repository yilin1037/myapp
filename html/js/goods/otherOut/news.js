...
var dataListThree = {
	...
	,cols: [[ 
		{type:'checkbox'}
		,{type:'numbers', width:50, title: '序号'}
		,{field:'pic_path', width:150, title: '图片'}
		,{field:'prd_no', minWidth:180, title: '商品编码'}
		,{field:'prd_sku_no', minWidth:180, title: 'SKU编码'}
		,{field:'sku_name', minWidth:200, title: '属性'}
		,{field:'sort_name', width:100, title: '商品简称'}
...
var dataLoadThree = {
	...
	tableLoadTable:function(){
		...
		layer.load(2);
		$.ajax({
			url:'?m=WMS&c=Storage&a=getPrdNewList',
			dataType: 'json',
			type: "post",
			data:{
				page:1,
			...
			success:function(data){
				...
				if(data.data){
					...
					if(!dataListThree.tableObj){
						for(var i=0;i<data.data.length;i++){
							data.data[i].change = 0;
							data.data[i].numIndex = i;
							data.data[i].pic_path = "<img src="+data.data[i].pic_path+">";
							dataListThree.data.push(data.data[i]);
						}
						flow.OneTableObj= table.render(dataListThree);
					}else{
						dataListThree.data = [];
						for(var i=0;i<data.data.length;i++){
							data.data[i].change = 0;
						...
						dataLoadThree.tableObj.reload(dataListThree);
					}
				}else{
					table.render(dataListThree);
				}
			}
		})
	}
...
function demo(){
	$.ajax({
		url:'/?m=goods&c=otherOut&a=get_document_no',
		dataType: 'json',
		type: "post",
		data:{act:act},
		success:function(data){
