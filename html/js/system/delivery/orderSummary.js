...
var flow = new Vue({
	...
	methods: {
				...
				if(who == "refunds"){	
					if(self.special == 'tuikuan'){																																						//===========
						...
						$("#searchArr").append("<span class='add refu rem'>申请退款<i class='dele' id='specialGroup' onclick='closeNow(\"specialGroup\")'></i></span>");							//===========
																																					//===========
					}else if(self.special == 'nospecial'){																																					//===========																																						//===========
						$("#searchArr .refu").remove();																																				//===========
						$(".specialGroup .ic").remove();																																			//===========
						$(toggle).removeClass("border");																																		//===========
					}																																												//===========
				}else if(who == "notuikuan"){
																																								//===========
        ...
        setExcel: function(obj) {
            if (obj == 'export') {
            	layer.open({																																											//===========
					...
					area: ['90%', '90%'], //宽高																																					//===========
					shade: 0.3,																																											//===========
					content: '?m=system&c=takeTemplate&a=index',																																							//===========
					cancel: function (index, layero) {																																					//===========
																																																		//===========
					}																																													//===========
			});	
		...
		resetNow:function(){
			...
			shippingChange('');
			var option ='';
			option ='<option value="" selected="selected">-是否退款-</option><option value="notuikuan" >不含申请退款</option><option value="tuikuan">含申请退款</option>';
			$("#special").html(option);
			$("#yufahuo").prop("checked",false);
		},
		select_tuikuan:function(event){
			var self = this;
			var value_s =event.target.value;
			self.special=value_s;
			var toggle = event.currentTarget;
			$(".specialGroup div").each(function(){																																				//===========
				$(".specialGroup .ic").remove();																																				//===========
				$(this).removeClass("border");																																			//===========
			...
			//self.searchAdd('specialGroup','refunds');
			// if(value_s=='tuikuan'){
			// }else{
			searchALLNow(self,'F');
			// }
		},
		check_sele:function(event){
			var self = this;
			self.yufahuo=event.target.checked;
			var toggle = event.currentTarget;
			$(".statusGroup div").each(function(){																																				
				$(".statusGroup .div.ic").remove();																																					
				$(this).removeClass("border");	
			...
			$(toggle).append("<i class='ic'></i>");																															
			$(toggle).addClass("border");																																				
			if(self.yufahuo == true){																																						
				self.singleStatus = 0;																																						
				$("#searchArr .equip").remove();																																			
				$("#searchArr").append("<span class='add equip rem'>预发货<i class='dele' id='statusGroup' onclick='closeNow(\"statusGroup\")'></i></span>");									
...
//$("#layCollectGoods").val(0);//-----初始化选择框
		...
		exportTakeGoods:function(){
			...
			var type ={
					'yufahuo':self.yufahuo==false?"1":"0",
...
function searchALLNow(self,page,callback){
	...
	var dateBegin = $("#dateBegin").val();																																							
	var dateEnd = $("#dateEnd").val();	
	var daishenhe = $("#daishenhe").is(":checked");
	var daifahuo = $("#daifahuo").is(":checked");
	var yufahuo = self.yufahuo;
	/***********/
	var orderSelect = $("#separator1").val();
	...
	var data = {
		...
		'yufahuo':yufahuo==false?'1':'0'
	};	
	self.searchData = data;
	$.ajax({
		url: "/index.php?m=system&c=delivery&a=getGoodsTrade",
		type: 'post',
		data: {data: data},
		dataType: 'json',
		success: function (data) {
			...
			var pageSize = data.pageSize;
			var result_total = data.result_total;
			self.result_total = data.result_total;
			self.cumulative_orders = data.cumulative_orders
			self.total_quantity = data.total_quantity;
			self.total_goods = data.total_goods;
