var tableList = new Vue({
	...
	mounted: function() {
		...
		$.ajax({
			url: "/index.php?m=system&c=printShip&a=getDsosConfig",
			type: 'post',
			data: {},
			dataType: 'json',
			async:false,
		...
		$.ajax({
			url: "/index.php?m=system&c=printShip&a=getAddressConfig",
			type: 'post',
			data: {},
			dataType: 'json',
			async:false,
			success: function (data) {
				if(data.configValue == "yes"){
					self.addressCheckt = "yes";
					$("#addressCheckt").iCheck('check');
					$("#addressInput").val(data.configKeyValue);
				}
				// if(data.scanMessageSingle == "yes"){
				// 	self.scanMessageSingle = "yes";
				// 	$("#addressCheckt").iCheck('check');
	...
	methods: {
		...
		dsosPrinterSetup:function(){
			...
			var self = this;
			$.ajax({
				url:'/?m=system&c=printShip&a=getTemplateDsos',
				dataType: 'json',
				type: "post",
				data:{},
				success:function(data){
			...
			layer.open({
				...
				yes: function(index, layero){
					...
					$.ajax({
						url:'/?m=system&c=printShip&a=dsosTplBqLayprint',
						dataType: 'json',
						type: "post",
						data:{
							layprintTplBq:layprintTplBq,
		...
		getTid:function(){
			...
			if(event.keyCode == 13){
				...
				if(gridItem.length == 0){
					self.order = [];
					$.ajax({	
						url: "/index.php?m=system&c=printShip&a=getData",		
						type: 'post',
						...
						success: function (data) {
							...
							if(onlySendWl == 'yes' && onlySendWlList.indexOf(',' + data.order[0].type.replace('DF_','')+ '|' + data.order[0].express_form + ',') == -1){
								if(data.order[0] && data.order[0].new_tid){
									uniqueLog(data.order[0].new_tid,unique_code,'onlySendWl');
								}
							...
							{
								if(data.order[0] && data.order[0].new_tid){
									uniqueLog(data.order[0].new_tid,unique_code,'error');
								}
							...
							if(data.order != 0){
								...
								if(scanSendSingleCheck == "on"){//单品验货
									...
									for(var i = 0; i<item.length; i++){
										if(item[i]['unique_code'] == unique_code || item[i]['serial_no'] == unique_code){
											...
											{
												$.ajax({																			
													url: "/index.php?m=system&c=printShip&a=setSerialNoCheckOut",	
													type: 'post',
								...
								if(refund_status == "WAIT_SELLER_AGREE" && send_status != "WAIT_SENDED"){
									...
									uniqueLog(tid,unique_code,'refund');
									//$("#btn2").addClass("layui-btn-disabled");
									//$("#btn3").addClass("layui-btn-disabled");
								}else{
									if(send_status == "WAIT_SENDED"){
										...
										uniqueLog(tid,unique_code,'success');
										layui.use('layer', function(){
										  var layer = layui.layer;
											layer.msg("通过",{
												icon: 1,
								...
								if(data.code != 'error')
								{
									...
									tableList.audioObj = speckText("无效的唯一码");
									$("#btn2").addClass("layui-btn-disabled");
									$("#btn3").addClass("layui-btn-disabled");
								}else{
									if(sendMark == "T"){
					...
					{
						$.ajax({																			
							url: "/index.php?m=system&c=printShip&a=getSerialNoPrdSkuId",	
							type: 'post',
					...
					if(uniqueMark == 'F'){
						...
						uniqueLog(self.tid,unique_code,'error');
						return false;
					}
					...
					if(refundMark == 'T'){
						tableList.audioObj = speckText("申请退款货品");
						uniqueLog(self.tid,unique_code,'refund');
					}else{
						uniqueLog(self.tid,unique_code,'success');
		...
		sendOrder:function(tid,type){
			...
			$.ajax({																			
				url: "/index.php?m=system&c=printShip&a=sendOrder",	
				type: 'post',
				...
				success: function (data) {
						if(data){
							if(data.code == "ok"){
								...
								$.ajax({							
									url: "/index.php?m=system&c=printShip&a=changeMark",
									type: 'post',
		...
		splitOrder:function(){
			...
			if(self.addressCheckt=='yes'){
				var addressInput = $("#addressInput").val();
				if(addressInput==''){
					layui.use('layer', function(){
					    var layer = layui.layer;
						layer.msg("请输入地址含有的关键字",{
							icon: 2,
							time: 2000
						});
					});
			...
			if(isSplit1 == "T" && isSplit2 == "T"){
				execAjax({
					...
					c:'printShip',
					a:'splitOrder',
					data:{unique_codes: unique_codes, tid: tid, system_id: self.system_id,addressInput:addressInput},
					success:function(data){
						$("#btn3").attr("disabled",false);
						$("#btn3").removeClass("layui-btn-disabled");
						if(data.code == "ok"){
		...
		print1Judge:function(){
			var self = this;
			$.ajax({														
				url: "/index.php?m=system&c=printShip&a=print1Judge",			
				type: 'post',															
				data: {},
				dataType: 'json',
				success: function (data) {
		...
		ajaxKeyword :function(){
			var self = this;
			var addressInput = $("#addressInput").val();
			
			$.ajax({																			
				url: "/index.php?m=system&c=printShip&a=addressLable",
				type: 'post',
				data: {keyWord: addressInput,addressCheckt:self.addressCheckt},
				dataType: 'json',
				success: function (data) {
					if(self.addressCheckt=='no'){
						$("#addressInput").val('');
					}
				}																							
			});
...
function uniqueLog(tid,code,status){
	var system_id = tableList.system_id;
	$.ajax({																			
		url: "/index.php?m=system&c=printShip&a=checkItemsLog",
		type: 'post',
		data: {tid: tid, code: code, status: status, system_id: system_id},
		dataType: 'json',
		success: function (data) {
			if(data.imgurl){
				$("#codeImg").attr("src",data.imgurl);
...
$(document).ready(function(){
	...
	//地址关键字
	$('#addressCheckt').on('ifChecked ifUnchecked', function(event){
		if (event.type == 'ifChecked') {
			tableList.addressCheckt = "yes";
		} else {	
			tableList.addressCheckt = "no";
		}	
		tableList.ajaxKeyword();
	});	
	...
	$('#printDsosLable').on('ifChecked ifUnchecked', function(event){
		...
		$.ajax({
			url: "/index.php?m=system&c=printShip&a=setPrintDsosLable",
			type: 'post',
	...
	$('#scanMessageSingle').on('ifChecked ifUnchecked', function(event){
		...
		$.ajax({																			
			url: "/index.php?m=system&c=printShip&a=setScanMessageSingle",
			type: 'post',
...
function layprintTplBqDsosChoose(){
	...
	//console.log(layprintTplBq);
	$.ajax({
		url:'/?m=system&c=printShip&a=getLayprintDsosChoose',
		dataType: 'json',
		type: "post",
		data:{
			layprintTplBq:layprintTplBq
