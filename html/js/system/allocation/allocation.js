	...
	$.ajax({
		...
		success: function (data) {
			...
			if(data.code == "no_sku"){											//如果选择的商品没有sku属性，直接赋值给  flow.items
				$("#good_name").val("");
				$("#good_name").attr("placeholder",data.result[0].title);
...
function skuChange(value){
	$.ajax({
		...
		data: {prd_sku_id:value,wh:$("#out").val()},
		dataType: "json",
		type: "POST",
		success: function (data) {
			if(data){
				$("#good_name").val("");
				$("#good_name").attr("placeholder",data.title+"_"+data.sku_name1+((data.sku_name1 && data.sku_name2) ? ";" : '')+data.sku_name2);
				var newData = {
					"discount":"无",
...
function settlement_sure(type){
	...
	newData.out_house = $("#out").val();
	
	newData.in_house = $("#in").val();
	...
	$.ajax({
		url: "/index.php?m=system&c=allocation&a=setData",
