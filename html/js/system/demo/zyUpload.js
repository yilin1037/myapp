	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "/?m=goods&c=association&a=orderImgUpload2",  	      // 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					// html += '			<button type="button" id="fileSubmit" class="upload_submit_btn" style="display: none;">确认上传文件</button>';
					// html += '		</div>';
				...
				if(files.length>1){
					...
					for (var i = 0, file; file = files[i]; i++) {
						...
						if (file.size >= 51200000) {
							$("#input-area").val('您这个"'+ file.name +'"文件大小过大');

						}else if(file.type != "image/jpeg" && file.type != "image/png") {
							$("#input-area").val('您这个"'+ file.name +'"格式必须是jpg或者png 格式文件');
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						para.onSelect(allFiles, allFiles);  // 回调方法
						...
						var funDealtPreviewHtml = function() {
							...
							if (file) {
								...
								var url = "/?m=goods&c=association&a=orderImgUpload2";
								var xhr = new XMLHttpRequest();
								var formData = new FormData();
								formData.append("uploadFile", file);
								xhr.open("POST", url, true);
								xhr.send(formData);
								$("#input-area").val("复制中 请等待！");
								xhr.onreadystatechange = function () {
									if (xhr.readyState == 4 && xhr.status == 200) {
										var _data = JSON.parse(xhr.responseText);
										$('#pages1-pic_path').val(_data.pic_name);
										$('#pages11-pic_path').val(_data.pic_name);
										if(_data.code=='ok'){

											$("#img").attr("src",_data.pic_name);
											$("#input-area").val("拖拽并上传完成！！");
					...
					onDelete: function(file, files) {
						...
						////console.info.info("剩下的文件");
						////console.info.info(files);
					},
