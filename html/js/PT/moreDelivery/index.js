...
function timingDelivery( state ){
	...
	if(setTime<100 && state=='F'){
		...
		oDi = setTimeout(function(){
			$.ajax({
				url:'?m=PT&c=moreDelivery&a=timingDelivery',
				dataType: 'json',
				type: "post",
				data:{
					express_no:express_no,
				...
				success:function(data){
					if(data['code'] == 'ok'){
						...
						if(flow.printNow == 1 && data['new_tid'] != ''){
							miandan(data['new_tid'], data['express_type']);
						}else if(flow.printNow == 1 && data['new_tid'] == ''){
							speckText('打印失败');
							layer.msg('打印失败',{
								icon: 2,
								time: 2000
...
function miandan(new_tid,express_type, ForcePrint){
	...
	$.ajax({
		url: "/index.php?m=PT&c=moreDelivery&a=printNow",			
		type: 'post',															
