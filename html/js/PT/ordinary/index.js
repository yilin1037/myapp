...
layui.use(['laydate', 'form', 'laypage', 'layer', 'upload', 'element', 'table'], function(){
	...
	doGetPrinters(function(data){
		...
		var LastSelectedPrinterName = window.localStorage.getItem("LastSelectedPrinterName");
		flow.layprint = data;
		var ohtml = "";
		for(var i=0;i<data.length;i++){
			if(LastSelectedPrinterName == data[i].name){
				ohtml += "<option value='"+data[i].name+"' selected >"+data[i].name+"</option>";
			}else{
	...
	//监听打印机选中事件
	form.on('select(layprint)', function(data){
		//alert(JSON.stringify(data));
		var PrinterName = data.value;
		//保存数据语法：
		window.localStorage.setItem("LastSelectedPrinterName", PrinterName);
		//读取数据语法：
		//console.log(window.localStorage.getItem("LastSelectedPrinterName"));
...
$("#saveInto").click(function(){
	...
	if(!onOff){
		...
		$.ajax({
			url:'?m=PT&c=Storage&a=setInOrdinary',
			dataType: 'json',
			type: "post",
			data:{
				wh:wh,
...
function searchBtn(){
	...
	$("#saveInto").prop("disabled",false);
	$("#saveInto2").prop("disabled",false);
	dataLoadOne.tableLoadTable();
}
