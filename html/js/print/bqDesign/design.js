...
function addField(param){
    ...
    switch(param['TYPE']){
		...
		case 'expire_day':
            newFieldHtml = "<div id='"+param['ID']+"' class='field isMultiple'>"
                +"<div class='conent' style='text-align: center;background-color:#000;color:#fff;'>"
                   +param['VALUE']
                +"<div>"
            +"</div>";
        break;
...
function setPrintData(FIELD,VALUE){
    ...
    param['VALUE'] = VALUE;
    param['FIELD_TYPE'] = '';
    param['TYPE'] = FIELD == 'expire_day' ? 'expire_day' : 'field';
    param['newLeft'] = 0;
    param['newTop'] = 0;
    param['newWidth'] = 150;
    param['newHeight'] = 20;
	...
	if(FIELD == 'unique_code_qr'){
		param['FIELD'] = 'unique_code';
        param['STYLE'] = {
            'left':'0px',
            'bottom':'0px',
            'width':'100%',
