...
function createPage(data){
    ...
    var TEMPLATE_PT_BG_IMAGE = paramObj['TEMPLATE_PT_BG_IMAGE'];
    $("#CUSTOM_AREA").css({"background-image":"url('"+TEMPLATE_PT_BG_IMAGE+"')","opacity":0.9});
    $("#CNPrintDesigner_DrawPanel").css({
        width : (pageWidth+'mm'),
    ...
    switch(param['TYPE']){
        case 'field':
            ...
            newFieldHtml = "<div id='"+param['ID']+"' class='field isMultiple'>"
                +"<div class='conent'>"
                    +(param['FIELD'] == 'expire_code' ? param['VALUE'] : ("#"+param['VALUE']+"#"))
                +"<div>"
            +"</div>";
        break;
        case 'text':
...
function cbPrintView(data){
    var width = $("input[name='width']").val() * 3;
    var height = $("input[name='height']").val() * 3;
    layer.open({
        type: 1
        ,title: false //不显示标题栏
        ,closeBtn: false
