...
var tableList = new Vue({
			...
			layui.use(['table','element','layer','form','laydate','tree'], function(){
				...
				var $ = layui.$, active = {
					reload: function(){
						...
						searchdata.status = $("#aftersale_status").val();
				...
				tab = table.render({
					elem: '#LAY_table_user'
					,url:'index.php?m=afterSale&c=aftersaleSuccess&a=getData'
					,skin: 'row'
					,page: true 
					,limits: [10, 20, 100]
					,limit: 10 
					...
					,cols: [[
						...
						,{field:'sh_no', title: '售后单号',"minWidth":120,fixed: true, event: 'rowClick'}
						,{field:'status_name', title: '处理类型',"width":90}
						,{field:'remark', title: '处理说明',"width":90}
						,{field:'sh_type_name', title: '售后类型',"width":90}
						,{field:'shopname', title: '来源店铺',"minWidth":100, event: 'rowClick'}
						,{field:'new_tid', title: '订单号',"minWidth":150, event: 'rowClick'}
						,{field:'buyer_nick', title: '买家昵称',"minWidth":100, templet: '#buyer_nickTpl', event: 'rowClick'}
						,{field:'refund_type', title: '退件类型',"width":90, event: 'rowClick'}
						,{field:'title', title: '问题原因',"minWidth":150, event: 'rowClick'}
				...
				table.on('tool(user)', function(obj){
					...
					if(obj.event === 'rowClick'){
						$.ajax({
							url: "/index.php?m=afterSale&c=aftersaleSuccess&a=getAfterSaleDetail",										
							type: 'post',																		
							data: {sh_no: sh_no},																	
							dataType: 'json',																	
							success: function (data) {			
						...
						$.ajax({
							url: "/index.php?m=afterSale&c=aftersaleSuccess&a=actionExchangeCheck",										
							type: 'post',																		
							data: {sh_no: sh_no},																	
							dataType: 'json',																	
							success: function (data) {
	...
	methods: {
		...
		actionComplete: function(index,sh_no,remark){
			...
			$.ajax({																																														
				url: "/index.php?m=afterSale&c=aftersaleSuccess&a=actionComplete",																																		
				type: 'post',																																												
		...
		addAfterSale:function(plan){
			...
			layer.open({
				...
				yes: function(index, layero){
					...
					$.ajax({																																														
						url: "/index.php?m=afterSale&c=aftersaleSuccess&a=addAfterSaleCheck",																																		
						type: 'post',																																												
						data: {tid: tid},																																													
						dataType: 'json',																																											
						success: function (data) {
		...
		afterSaleRefund: function(tid,extra_amtn,alipay_no){
			var self = this;
			$.ajax({																																														
				url: "/index.php?m=afterSale&c=aftersaleSuccess&a=afterSaleRefund",																																		
				type: 'post',																																												
...
function outputExcel(){
	var searchdata = tableList.searchData;
	searchdata.shopid = $("#shopname").attr("name");
	searchdata.sh_no = $("#aftersale_sh_no").val();
	searchdata.express = $("#aftersale_express").val();
	searchdata.express_no = $("#aftersale_express_no").val();
	...
	searchdata.buyer_nick = $("#aftersale_buyer_nick").val();
	searchdata.dateBegin = $("#dateBegin").val();
	searchdata.dateEnd = tableList.dateEnd;
	
	tableList.searchData = searchdata;
	var time = new Date().getTime();
	$.ajax({
		url: "/index.php?m=afterSale&c=aftersaleSuccess&a=outputExcel&loginact=file",
		type: 'post',
		data: {
			data: searchdata,
			time: time,
