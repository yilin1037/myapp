...
var tableList = new Vue({
	...
	mounted: function() {
		...
		//获取网店
		$.ajax({
			url: "/index.php?m=afterSale&c=aftersaleRefund&a=getShop",										
			type: 'post',																		
			data: {},																	
			dataType: 'json',																	
		...
		$(document).ready(function(){
			layui.use(['table','element','layer','form','laydate','tree'], function(){
				...
				tab = table.render({
					elem: '#LAY_table_user'
					,url: 'index.php?m=afterSale&c=aftersaleRefund&a=getData'
					,cols: [[
				...
				table.on('tool(user)', function(obj){
					...
					if(obj.event === 'rowClick'){
						$.ajax({
							url: "/index.php?m=afterSale&c=aftersaleRefund&a=getAfterSaleDetail",										
							type: 'post',																		
							data: {sh_no: sh_no},																	
							dataType: 'json',																	
							success: function (data) {			
						...
						$.ajax({
							url: "/index.php?m=afterSale&c=aftersaleRefund&a=actionExchangeCheck",										
							type: 'post',																		
							data: {sh_no: sh_no},																	
							dataType: 'json',																	
							success: function (data) {
	...
	methods: {
		...
		actionComplete: function(index,sh_no,remark){
			...
			$.ajax({																																														
				url: "/index.php?m=afterSale&c=aftersaleRefund&a=actionComplete",																																		
				type: 'post',																																												
		...
		addAfterSale:function(plan){
			...
			layer.open({
				...
				yes: function(index, layero){
					...
					$.ajax({																																														
						url: "/index.php?m=afterSale&c=aftersaleRefund&a=addAfterSaleCheck",																																		
						type: 'post',																																												
						data: {tid: tid},																																													
						dataType: 'json',																																											
						success: function (data) {
		...
		afterSaleRefund: function(tid,extra_amtn,alipay_no){
			var self = this;
			$.ajax({																																														
				url: "/index.php?m=afterSale&c=aftersaleRefund&a=afterSaleRefund",																																		
				type: 'post',																																												
